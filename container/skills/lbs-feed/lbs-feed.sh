...
case "${1:-help}" in
  ...
  get)
    ...
    if [ -z "$RESULT" ] || [ "$RESULT" = "null" ]; then
      echo "Ad $AD_ID is not in our current catalog. It may have been sold or removed. Suggest the customer browse landbrukssalg.no directly or search for similar items."
    else
      echo "$RESULT"
    fi
    ;;
